...
async function loadArticles() {
  ...
  try {
    ...
    async function render(list) {
      ...
      if (isMobile) {
        ...
        list.forEach(article => {
          ...
          if (t >= mondayThis && t < mondayNext) {
            (weeks.current[ymdKey(d)] ||= []).push(article);
          } else if (t >= mondayPrev && t < mondayThis) {
            (weeks.previous[ymdKey(d)] ||= []).push(article);
          } else {
            const wk = startOfWeekMonday(d);
            (weeks.others[ymdKey(wk)] ||= {});
            (weeks.others[ymdKey(wk)][ymdKey(d)] ||= []).push(article);
          }
        });
        ...
        async function renderWeek(title, daysMap) {
          ...
          for (const dayKey of sortedDays) {
            ...
            dayBlock.innerHTML = `<h3 class="day-title">${d.toLocaleDateString("fr-FR", { day: "numeric", month: "long" })}</h3>`;
        ...
        const others = Object.keys(weeks.others).sort((a, b) => new Date(b) - new Date(a));
        for (const wk of others) {
          const start = new Date(wk);
          const end = addDays(start, 6);
          await renderWeek(
            `Semaine du ${start.toLocaleDateString("fr-FR", { day: "numeric", month: "long" })} au ${end.toLocaleDateString("fr-FR", { day: "numeric", month: "long" })}`,
            weeks.others[wk]
          );
        }
      }
    ...
    function buildCategories(list, active = "Tous") {
      ...
      // === Style stroke + correctif Firefox ===
